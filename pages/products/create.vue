<template>
  <div class="container py-4">
    ...
    <form @submit.prevent="submitForm" class="product-form mx-auto">
      ...
      <div class="mb-3">
        <label class="form-label required">商品編號</label>
        <input v-model="form.productCode" type="text" class="form-control" :class="{'is-invalid': errors.productCode}" placeholder="例如：ORG-001" />
        <div v-if="errors.productCode" class="invalid-feedback">{{ errors.productCode }}</div>
      </div>
      <div class="mb-3">
        <label class="form-label required">品目名稱</label>
        <select v-model="form.itemName" class="form-select" :class="{'is-invalid': errors.itemName}">
          <option value="">請選擇</option>
      ...
      <div class="mb-3">
        ...
        <div class="ms-3">
          <div class="form-check form-check-inline" v-for="crop in cropOptions" :key="crop">
            <input class="form-check-input" type="checkbox" :id="'crop-' + crop" :value="crop" v-model="form.crops" />
      ...
      <div class="mb-3">
        ...
        <div v-for="(section, idx) in form.introSections" :key="idx" class="mb-2 border rounded p-2 bg-light position-relative">
          <div class="d-flex align-items-center mb-2">
            <select v-model="section.type" class="form-select w-auto me-2" style="min-width:90px">
              <option value="text">文字</option>
              <option value="image">圖片</option>
            </select>
            <button type="button" class="btn btn-sm btn-outline-secondary me-1" @click="moveSection(idx, -1)" :disabled="idx===0">↑</button>
            <button type="button" class="btn btn-sm btn-outline-secondary me-1" @click="moveSection(idx, 1)" :disabled="idx===form.introSections.length-1">↓</button>
          ...
          <div v-else>
            <input type="file" class="form-control" accept="image/*" @change="e=>onIntroImageChange(e, idx)" />
            <div v-if="section.imageUrl" class="mt-2">
              <img :src="section.imageUrl" alt="預覽" style="max-width:180px; border-radius:8px;" />
            </div>
          </div>
        </div>
        <button type="button" class="btn btn-outline-main w-100 mt-2" @click="addSection">＋ 新增段落</button>
      </div>
      <div class="mb-3">
      ...
      <div class="mb-3">
        ...
        <input type="file" class="form-control" multiple accept="image/*" @change="onImagesChange" :disabled="form.images.length>=4" />
        <div class="d-flex flex-wrap mt-2 gap-2">
          <div v-for="(img, idx) in form.images" :key="idx" class="position-relative">
            <img :src="img.url" alt="預覽" style="max-width: 120px; border-radius: 8px;" />
            <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0" style="transform:translate(40%,-40%)" @click="removeImage(idx)">×</button>
          </div>
        </div>
        <div class="form-text">最多上傳4張，最少1張</div>
      </div>
      <div class="mb-3">
        <label class="form-label">生產紀錄</label>
        <div v-for="(url, idx) in form.productionRecords" :key="idx" class="input-group mb-1">
          <input v-model="form.productionRecords[idx]" type="url" class="form-control" placeholder="貼上網頁網址" />
          <button type="button" class="btn btn-outline-danger" @click="removeProductionRecord(idx)">刪除</button>
        </div>
        <button type="button" class="btn btn-outline-main btn-sm mb-2" @click="addProductionRecord" :disabled="form.productionRecords.length>=5">＋ 新增網址</button>
        <div class="form-check mt-2">
          <input class="form-check-input" type="checkbox" id="showProductionRecord" v-model="form.showProductionRecord" />
          <label class="form-check-label" for="showProductionRecord">顯示於前台</label>
        </div>
      </div>
...
const form = reactive({
  ...
  crops: [] as string[],
  introSections: [
    { type: 'text', content: '', file: undefined as File | undefined, imageUrl: '' }
  ],
  validDate: '',
  spec: '',
  images: [] as { file: File, url: string }[],
  productionRecords: [''],
  showProductionRecord: true
})
...
function onFileChange(e: Event) {
  ...
  if (file) {
    // 將單一圖片包裝成物件並加入 images 陣列（最多 4 張）
...
function addSection() {
  form.introSections.push({ type: 'text', content: '', file: undefined as File | undefined, imageUrl: '' })
}

function removeSection(idx: number) {
  form.introSections.splice(idx, 1)
}

function moveSection(idx: number, dir: number) {
  const newIdx = idx + dir
  if (newIdx < 0 || newIdx >= form.introSections.length) return
  const temp = form.introSections[idx]
  form.introSections[idx] = form.introSections[newIdx]
  form.introSections[newIdx] = temp
}

function onIntroImageChange(e: Event, idx: number) {
  const file = (e.target as HTMLInputElement).files?.[0]
  if (file) {
    form.introSections[idx].file = file
    form.introSections[idx].imageUrl = URL.createObjectURL(file)
    form.introSections[idx].content = ''
  }
}
...
function onImagesChange(e: Event) {
  ...
  if (!files) return
  for (let i = 0; i < files.length && form.images.length < 4; i++) {
    const file = files[i]
    form.images.push({ file, url: URL.createObjectURL(file) })
  }
}
...
function addProductionRecord() {
  if (form.productionRecords.length < 5) form.productionRecords.push('')
}

function removeProductionRecord(idx: number) {
  form.productionRecords.splice(idx, 1)
  if (form.productionRecords.length === 0) form.productionRecords.push('')
}
...
function submitForm() {
  ...
  errors.productCode = form.productCode ? '' : '請輸入商品編號'
  errors.itemName = form.itemName ? '' : '請選擇品目名稱'
  errors.property = form.property ? '' : '請選擇性狀'
  if (errors.brandName || errors.companyName || errors.registrationNo || errors.productCode || errors.itemName || errors.property) return
  alert('送出成功！(可串接API)')
}
</script>
